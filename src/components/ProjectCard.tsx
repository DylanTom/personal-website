...
function ProjectCard(props: TProject) {
	...
	if (hover) {
		return (
			<div className={"relative flex flex-col shadow-md hover:shadow-xl hover: cursor-pointer justify-center items-center m-6 border-2 rounded-xl"} onMouseEnter={e => showOptions(e)} onMouseLeave={e => hideOptions(e)}>
				<div className="flex flex-row space-x-20 justify-center items-center p-6 bg-[#f5f5f5] w-full h-full">
					<div className="flex">
					...
					<div className="flex">
						<img src={seeMore} width="48px" height="48px" onClick={() => setIsOpen(true)} />
	...
	// 			</div>
	// 		</div>
	// 		<div className={hover ? "flex flex-row space-x-20 justify-center items-center p-6 bg-[#f5f5f5] w-full h-full inset-0 object-cover" : "hidden"}>
	// 				<div className="flex">
